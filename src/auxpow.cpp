...
int CMerkleTx::GetDepthInMainChain(const CBlockIndex* &pindexRet) const
{
    ...
    // v0.21: Use ChainActive().Tip() to search
    const CBlockIndex* pindex = ::ChainActive().Tip();
    while (pindex && pindex->GetBlockHash() != hashBlock) {
        pindex = pindex->pprev;
    }
...
// v0.21: AcceptToMemoryPool signature changed significantly, commented out for now
// bool CMerkleTx::AcceptToMemoryPool(const CAmount& nAbsurdFee)
// {
//     // Need to update to new AcceptToMemoryPool API
//     return false;
// }
               ...
               const Consensus::Params& params) const
{
    ...
    CScript::const_iterator pcHead =
        std::search(script.begin(), script.end(), std::begin(pchMergedMiningHeader), std::end(pchMergedMiningHeader));
    ...
    if (pcHead != script.end())
    {
        ...
        if (script.end() != std::search(pcHead + 1, script.end(), std::begin(pchMergedMiningHeader), std::end(pchMergedMiningHeader)))
...
CAuxPow::initAuxPow (CBlockHeader& header)
{
  ...
  header.auxpow = std::make_shared<CAuxPow>(coinbaseRef);
  ...
  // Copy parent block header fields to CPureBlockHeader
  header.auxpow->parentBlock.nVersion = parent.nVersion;
  header.auxpow->parentBlock.hashPrevBlock = parent.hashPrevBlock;
  header.auxpow->parentBlock.hashMerkleRoot = parent.hashMerkleRoot;
  header.auxpow->parentBlock.nTime = parent.nTime;
  header.auxpow->parentBlock.nBits = parent.nBits;
  header.auxpow->parentBlock.nNonce = parent.nNonce;
}
