                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetJunkcoinMinRelayFee(tx, nSize, !fLimitFree))
        {
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    CAmount blockReward = GetJunkcoinBlockSubsidy(pindex->nHeight, nFees, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
