...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    // Junkcoin: Ensure auxpow-compatible header (chain ID + base version 4)
    const Consensus::Params& consensus = chainparams.GetConsensus();
    pblock->SetChainId(consensus.nAuxpowChainId);
...
void BlockAssembler::addPackageTxs(int &nPackagesSelected, int &nDescendantsUpdated)
{
    ...
    while (mi != m_mempool.mapTx.get<ancestor_score>().end() || !mapModifiedTx.empty()) {
        ...
        // Bound the aggregate MWEB input count so placeholder inputs cannot
        // exhaust the block's input limit while weight remains low.
