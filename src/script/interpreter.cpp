...
bool EvalScript(std::vector<std::vector<unsigned char> >& stack, const CScript& script, unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptExecutionData& execdata, ScriptError* serror)
{
    ...
    // Junkcoin: largest magnitude representable by a default (4-byte)
    // CScriptNum. Re-enabled arithmetic opcodes that can grow their operands
    // are bounded by this so an out-of-range result fails deterministically
    // at the point of computation, instead of producing an oversized element
    // that only errors later (or not at all, if consumed non-numerically).
    ...
    try
    {
        for (; pc < pend; ++opcode_pos) {
            ...
            // Junkcoin: Check if legacy disabled opcodes are re-enabled via consensus flag.
            ...
            switch (opcode)
            {
                ...
                case OP_MAX:
                {
                    ...
                    case OP_RSHIFT:
                        ...
                        if (bn1 < bnZero || bn2 < bnZero || bn2.getint() >= 32)
                            return set_error(serror, SCRIPT_ERR_INVALID_STACK_OPERATION);
                        bn = CScriptNum(int64_t(bn1.getint()) >> bn2.getint());
                        break;
