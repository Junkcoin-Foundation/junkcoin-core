...
std::string EntryDescriptionString()
{
    return "    \"size\" : n,             (numeric) virtual transaction size as defined in BIP 141. This is different from actual serialized size for witness transactions as witness data is discounted.\n"
           "    \"fee\" : n,              (numeric) transaction fee in " + GetCurrencyUnit() + "\n"
...
UniValue getblockstats(const JSONRPCRequest& request) {
    ...
    ret_all.pushKV("subsidy", GetJunkcoinBlockSubsidy(pindex->nHeight, 0, Params().GetConsensus(pindex->nHeight), block.hashPrevBlock));
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "  \"value\" : x.xxx,           (numeric) The transaction value in " + GetCurrencyUnit() + "\n"
            "  \"scriptPubKey\" : {         (json object)\n"
            "     \"asm\" : \"code\",       (string) \n"
            "     \"hex\" : \"hex\",        (string) \n"
            "     \"reqSigs\" : n,          (numeric) Number of required signatures\n"
            "     \"type\" : \"pubkeyhash\", (string) The type, eg pubkeyhash\n"
            "     \"addresses\" : [          (array of string) array of junkcoin addresses\n"
