...
UniValue getrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw runtime_error(
            ...
            "     {\n"
            "       \"value\" : x.xxx,            (numeric) The value in " + GetCurrencyUnit() + "\n"
            "       \"n\" : n,                    (numeric) index\n"
    ...
    CTransactionRef tx;
    uint256 hashBlock;
    // junkcoin: Is this the best value for consensus height?
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the junkcoin address, the numeric value (can be string) is the " + GetCurrencyUnit() + " amount\n"
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"D731rRTrFydjJdZCKNzfB5go229p59GUGD\"   (string) junkcoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
